...
| 入口 | 说明 |
|------|------|
| **外部 AI harness** | canonical Skill 正在基于 SQLite CLI 重建；过时的项目级 `.claude/skills` 已删除。 |
| **Telegram** | 可选的移动端 / 远程访问机器人（`agent-telegram`），按需编译和运行。 |
| **Data CLI** | 脚本和未来 Agent Skill 使用的机器可读 SQLite 数据工具（`arcana-data`）。 |

桌面 UI 和内置 Rust Agent 在迁移期间仍使用旧 JSON services；`arcana-data` 已切到新的 Application/Repository/SQLite 栈，因此其数据暂时不会显示在桌面 UI 中。
...
- **数据**：迁移中的 UI/Agent 仍使用本地 JSON；`arcana-data` 使用本地 SQLite 与确定性 JSON import/export
...
`arcana-data init` 只创建 SQLite runtime 和 `basic` Pack，不会填充旧 UI 或新手任务。Record、Pack 与 Status 命令族已经迁移；等 SQLite Achievement、Mission 和 Memory 命令完成后，再恢复 canonical Agent Skill。
...
- **Agent 迁移尚未完成**：内置 CLI/Telegram Agent 仍使用旧 JSON；在 SQLite 命令面完整之前，canonical 外部 Skill 暂时移除。
