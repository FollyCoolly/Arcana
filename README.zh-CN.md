...
Status、Achievement、Skill 和 Mission 页面已经使用新的 Application/Repository/SQLite 栈。Items、Gallery 和内置 Rust Agent 在迁移期间仍使用旧 JSON services。
...
- **数据**：Status、Achievement、Skill、Mission 与 `arcana-data` 使用本地 SQLite；Items、Gallery 和内置 Agent 仍使用本地 JSON；确定性 JSON 是后续 Git 同步格式
...
`arcana-data init` 只创建 SQLite runtime 和 `basic` Pack，不会填充新手任务。Record、Pack、Status、Achievement、Arcana Skill 查询、Mission、AssistantMemory、紧凑 Agent 上下文、dry-run、原子用户状态 batch、contract fixtures 与 canonical 外部 Skills 已经迁移；Tauri Status、Achievement、Skill 和 Mission 页面也已使用该运行时，Items 与 Gallery 暂时保留外部/旧数据源。
...
- **当前处于分层迁移状态**：Tauri Status、Achievement、Skill、Mission 页面与 `arcana-data` 已使用 SQLite；Items、Gallery 与内置 Agent 仍使用旧 JSON/外部来源。
