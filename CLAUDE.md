...
Arcana 是一个 Persona 5 风格的游戏化人生管理桌面应用。核心用户数据统一使用 SQLite 本地运行时，并可通过确定性 JSON Codec 导入导出。

## 当前架构边界

- Tauri Status、Achievement、Skill、Mission 与 `arcana-data` 共用 `application/`、`domain/`、`storage/sqlite/`。
...
- Items、Gallery、Weather 只读取 `~/.arcana/data` 中的外部来源配置，不属于核心 SQLite 用户数据。
...
- 一个 SQLite runtime / 同步仓库对应一个用户，不建立 Profile 或 `profile_id`。
