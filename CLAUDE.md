...
- **调色板**: `#000000`、`#ffffff`、`#E5191C` + 数据可视化专用 `#F5A623`（`--rm-gold`），**不使用渐变**
...
所有数据操作的业务逻辑集中在 `src-tauri/src/services/` 下，被 Rust agent、MCP Server、Tauri commands 三条路径共用：
...
位于 `src-tauri/src/bin/mcp_server.rs`，基于 `rmcp` v1.3 + stdio 传输。Claude Code 通过 `.mcp.json` 自动连接。

```bash
cargo build --manifest-path src-tauri/Cargo.toml --bin mcp-server   # 构建
cargo run --manifest-path src-tauri/Cargo.toml --bin mcp-server      # 开发运行
```
...
| Tool | 类型 | 用途 |
|------|------|------|
| `get_context` | 读 | 读取当前状态概览 — **始终先调用** |
| `read_file` | 读 | 沙箱读取 data/ 下任意文件 |
| `update_mission` | 写 | 更新任务字段 / main_menu 配置 |
| `create_mission` | 写 | 插入新任务（phan-site 用） |
| `update_status` | 写 | 更新状态指标 |
| `update_achievement` | 写 | 追踪/达成成就 |
| `write_changelog` | 写 | 写审计日志（**MANDATORY**，caller 传 skill 名） |
| `update_mission_memory` | 写 | 更新 AI 记忆（不需 changelog） |
...
Skills 通过 MCP 工具操作数据，不直接读写 JSON 文件：

| Skill | 用途 | 触发方式 |
|-------|------|----------|
| `velvet-room` | 通用进度汇报：理解意图 → 调 MCP 工具更新数据 | `/velvet-room` |
| `phan-site` | 任务提案生成：基于上下文生成 3-5 个任务 | `/phan-site` |
| `pack-manager` | 成就包管理：创建 / 优化 / 扩展 achievement pack（暂未接入 MCP） | `/pack-manager` |
...
位于 `storage/` 下，被 agent tools、MCP Server、Tauri commands 共用。写入后自动校验，失败则回滚文件并返回错误。覆盖 agent 写的 4 类文件：
...
- **MCP Server 统一数据入口**: `services/` 层被 agent、MCP Server、Tauri commands 共用。Claude Code Skills 通过 MCP 工具操作数据，不直接读写 JSON
