...
- Tauri Status、Achievement、Skill 页面已通过 `commands/data_platform.rs` 使用新 Application/SQLite 运行时；其余 Tauri 页面与内置 Rust Agent 暂时仍使用 `models/`、`services/` 和旧 JSON storage。只修复旧模型的必要问题，不在其中增加新数据平台功能。
...
src-tauri/src/
  ...
  commands/                       新数据平台 IPC 与尚未迁移的 UI 旧命令
