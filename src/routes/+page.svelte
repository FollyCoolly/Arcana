<script lang="ts">
  ...
  const MENU_ITEMS: MenuItem[] = [
    { id: "status", label: "Status", description: "Body and life metrics from local JSON snapshots.", enabled: true },
    { id: "skills", label: "Skills", description: "Skill tree progression linked to achievements.", enabled: true },
  ...
  const MENU_QUAD_CONFIGS: { rot: number; clip: string }[] = [
    { rot: -35, clip: 'polygon(10% 25%, 70% 0%, 95% 99%, 10% 80%)' },
    { rot: -27, clip: 'polygon(1% 8%, 98% 2%, 97% 92%, 3% 98%)' },
    { rot: -20, clip: 'polygon(2% 0%, 99% 6%, 96% 96%, 0% 88%)' },
    { rot: -8,  clip: 'polygon(0% 10%, 100% 3%, 98% 94%, 2% 100%)' },
    { rot: -2,  clip: 'polygon(1% 4%, 97% 0%, 100% 90%, 3% 96%)' },
    { rot: 2,   clip: 'polygon(0% 6%, 98% 0%, 100% 100%, 2% 92%)' },
  ];
  ...
  <section class="rm-scene">
    ...
    {#if currentScreen === "achievements"}
      <AchievementsScreen
        onBack={goBack}
        {achievementData}
        onAchievementDataLoaded={(data) => { achievementData = data; }}
      />
    {/if}

    {#if currentScreen === "skills"}
      <SkillsScreen onBack={goBack} {achievementData} />
    {/if}
...
<style>
  ...
  .rm-sm-1 { background: var(--rm-white); transform: scale(0.50); }
  .rm-sm-2 { background: var(--rm-black); transform: scale(0.48); }
  .rm-sm-3 { background: var(--rm-white); transform: scale(0.38); }
  .rm-sm-4 { background: var(--rm-black); transform: scale(0.28); }
  .rm-sm-5 { background: var(--rm-white); transform: scale(0.18); }
  .rm-sm-6 { background: var(--rm-black); transform: scale(0.08); }
