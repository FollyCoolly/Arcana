...
Arcana 是个人信息的综合管理以及可视化工具，希望能用游戏化思维管理生活。给"地球Online"加上一个用户界面。

## 💡 设计理念

Arcana 不是另一个用于“打卡”的习惯追踪软件。作为桌面端软件，更强调信息的整理和综合。

虽然希望以游戏的方式管理生活，但希望呈现的数据有真实意义。因此不会有"今天干了某件事，某某属性+x"这样强行模仿RPG游戏的功能。

Arcana 希望能找到现实与游戏共性的地方，并加以呈现，但不会强行把现实往游戏的框架里套。

## 🧩 功能模块

作为"地球Online"的用户界面，Arcana 初步计划提供六个主要功能模块：
...
- 已支持读取 `user_profile.json` 显示用户名，并计算“游戏天数”（出生至今）。
- 已支持基于 `height_cm` 与 `weight_kg` 自动计算 BMI。
- 当前未实现趋势图与历史时序数据展示。
...
- 提供相关脚本，自动化更新你的阅读、观影、游戏数据，转换成Arcana 的 JSON 格式。
...
Arcana 目前采用纯 **Local-First** 的架构，所有数据存储于本地 JSON 文件中。
...
- [ ] **Status 模块开发**
  ...
  - [x] 实现 Rust 端 JSON 文件读取
  - [x] 实现 Tauri Command: `load_status_data`
  - [x] 创建 Status 页面 UI（先数据卡片，后续趋势图）
...
- [ ] 实现 Arcana MCP Server（Rust），暴露数据读写 tools
