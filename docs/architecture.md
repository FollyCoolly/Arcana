...
Reality Mod 采用 **Local-First 桌面应用** 架构，基于 Tauri v2 框架，结合 Rust 后端与 Svelte 前端。
...
Reality Mod 采用渐进式开发，目录结构随着开发阶段逐步扩展。以下是最终完整版本的目录结构：
...
Content Packs 是 Reality Mod 的核心特性，支持动态加载不同的成就包。
...
**结论**：Reality Mod 是本地工具，不需要复杂的网络功能，Tauri 的轻量化和高性能更合适。
...
- **规模**：Reality Mod 数据量不大，JSON 性能足够
