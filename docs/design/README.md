...
5. 进行中：`arcana-data init`、`record ...` 和 `json import|export` 已接入新运行时；统一结果 Envelope、结构化错误、dry-run/batch、其余 CLI、Tauri UI 和 canonical Agent Skill 待切换，切换完成后直接停止使用旧 JSON。
