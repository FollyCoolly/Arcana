...
flowchart TB
    ...
    Packs["Pack Definitions and Templates"]
...
Pack RecordSet Template --adopt--> Global RecordSet
Pack DimensionDefinition -> local UI selection
```

- RecordData 是事实源。
- Status 需要确定性计算；Achievement 允许 Agent 语义判断。
- Arcana Skill 只读取状态为 `achieved` 的成就，不读取 `tracked` 状态或 RecordData。
- 派生的分数、等级和进度不作为新的事实回写。

## 4. 用户边界

- 一个 Git 数据仓库、一个 SQLite 数据库对应一个用户。
- 不建立 Profile 实体或 `profile_id`。
- 可选 `UserSettings` 只包含 `nickname?` 和 `birth_date?`。
- 缺少昵称时显示产品默认值；缺少生日时不计算游戏天数。
...
| 模块 | 权威输入 | 持久化内容 | 派生内容 |
| --- | --- | --- | --- |
| Record | 用户明确提供的事实、导入和外部适配器 | RecordSet、RecordData | 聚合查询结果 |
| Status | RecordData + Pack DimensionDefinition | Pack 持有 Dimension 定义；本机配置只保存五个选择 ID | 子 Score、Dimension 分数、等级 |
| Achievement | 定义、RecordData、用户陈述 | 最小用户状态（`tracked` / `achieved`） | 即时进度说明 |
...
| Pack | 用户创建或导入的定义 | manifest、模板、Achievement、Skill | PackForest 视图 |
...
- Agent 可以从对话、日记或周记提取 RecordData，并判断 Achievement。
...
实现顺序应从领域模型、Repository 和同步 Codec 开始，再迁移 UI/CLI/Skill。不能先让某个模块直接使用新库、其他模块继续修改旧 JSON，从而形成两个权威数据源。
